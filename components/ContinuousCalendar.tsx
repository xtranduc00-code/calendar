...
import { expandRecurringEvents, type ExpandedEvent } from '@/lib/recurrence';
...
function dayStart(day: number, month: number, year: number): number {
  return new Date(year, month, day).setHours(0, 0, 0, 0);
}

function getExpandedSpanningDay(
  expanded: ExpandedEvent[],
  day: number,
  month: number,
  year: number
): { expanded: ExpandedEvent; isFirstDay: boolean; isLastDay: boolean }[] {
  if (month < 0) return [];
  const cellDayStart = dayStart(day, month, year);
  const result: { expanded: ExpandedEvent; isFirstDay: boolean; isLastDay: boolean }[] = [];
  for (const ex of expanded) {
    const evStartDay = dayStart(ex.start.getDate(), ex.start.getMonth(), ex.start.getFullYear());
    const evEndDay = dayStart(ex.end.getDate(), ex.end.getMonth(), ex.end.getFullYear());
    if (cellDayStart >= evStartDay && cellDayStart <= evEndDay) {
      result.push({
        expanded: ex,
        isFirstDay: cellDayStart === evStartDay,
        isLastDay: cellDayStart === evEndDay,
      });
    }
  }
  return result.sort((a, b) => a.expanded.start.getTime() - b.expanded.start.getTime());
}

const DEFAULT_EVENT_COLOR = '#3B82F6';

export const ContinuousCalendar: React.FC<ContinuousCalendarProps> = ({ onClick, events = [], onAddEventClick, onAllEventsClick }) => {
    ...
    const calendar = calendarWeeks.map((week, weekIndex) => (
      <div className="flex w-full" key={`week-${weekIndex}`}>
        {week.map(({ month, day }, dayIndex) => {
          ...
          const cellStart = month >= 0 ? new Date(year, month, day).getTime() : 0;
          const isPast = month >= 0 && cellStart < todayStart;

          const spanning = month >= 0 ? getExpandedSpanningDay(expandedEvents, day, month, year) : [];
            return (
            <div
              key={`${month}-${day}`}
            ...
            >
              ...
              {spanning.length > 0 && (
                <div className="absolute left-0 right-0 top-8 flex flex-col gap-0.5 px-1 sm:top-9 lg:top-11 lg:px-1.5">
                  {spanning.slice(0, 4).map(({ expanded: ex, isFirstDay, isLastDay }, i) => {
                    const ev = ex.event;
                    const bg = ev.color || DEFAULT_EVENT_COLOR;
                    const roundedLeft = isFirstDay ? 'rounded-l-md' : 'rounded-none';
                    const roundedRight = isLastDay ? 'rounded-r-md' : 'rounded-none';
                    return (
                      <div
                        key={`${ev.id}-${ex.start.getTime()}-${i}`}
                        className={`min-h-[18px] w-full truncate px-1.5 py-0.5 text-[10px] font-medium sm:min-h-[20px] sm:text-xs lg:min-h-[22px] lg:px-2 ${roundedLeft} ${roundedRight}`}
                  ...
                  {spanning.length > 4 && (
                    <span className="text-[10px] text-slate-500 sm:text-xs">+{spanning.length - 4}</span>
                  )}
                </div>
              )}
              {isNewMonth && (
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3">
            ...
            {onAllEventsClick && (
              <button
                ...
                className="whitespace-nowrap rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-center text-sm font-medium text-gray-900 hover:bg-gray-100 sm:rounded-xl lg:px-5 lg:py-2.5"
              >
                All events
              </button>
            )}
